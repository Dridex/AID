...
echo "Copying files..."
cp ./install-files/aid-master.py /opt/scripts/aid-master/
chmod 744 /opt/scripts/aid-master/aid-master.py
cp ./install-files/etc/controllers.conf /opt/scripts/aid-master/etc/
chmod 644 /opt/scripts/aid-master/etc/controllers.conf
cp ./install-files/etc/proxy.conf /opt/scripts/aid-master/etc/
chmod 644 /opt/scripts/aid-master/etc/proxy.conf
cp ./install-files/etc/logging.conf /opt/scripts/aid-master/etc/
chmod 644 /opt/scripts/aid-master/etc/logging.conf
cp ./install-files/helperMaster.py /opt/scripts/aid-master/
chmod 644 /opt/scripts/aid-master/helperMaster.py
echo "Done."

echo "Creating init script..."
if [ -d "/etc/init.d/" ]; then
   cp ./install-files/startup/aid-init-script.sh /etc/init.d/aid-master
   chmod 755 /etc/init.d/aid-master
fi
if [ -d "/etc/systemd/system" ]; then
   cp ./install-files/startup/aid-master.service /etc/systemd/system/aid-master.service
