...
echo "Copying files..."
cp -i ./install-files/aid-agent.py /opt/scripts/aid/
chmod 744 /opt/scripts/aid/aid-agent.py
cp -i ./install-files/etc/aid.conf /opt/scripts/aid/etc/
chmod 644 /opt/scripts/aid/etc/aid.conf
cp -i ./install-files/etc/logging.conf /opt/scripts/aid/etc
...
if [ -d "/etc/init.d/" ]; then
   cp -i ./install-files/startup/aid-init-script.sh /etc/init.d/aid
   chmod 755 /etc/init.d/aid
fi
if [ -d "/etc/systemd/system" ]; then
   cp -i ./install-files/startup/aid.service /etc/systemd/system/aid.service
   chmod 644 /etc/systemd/system/aid.service
fi
echo "Complete!"

while true; do
   read -p "Do you wish to overwrite the entire plugins directory? " yn
   case $yn in
      [Yy]* ) cp ./install-files/plugins/* /opt/scripts/aid/plugins/; break;;
      [Nn]* ) break;;
